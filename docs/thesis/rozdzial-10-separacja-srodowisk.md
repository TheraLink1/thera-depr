...
różnią się również między serwisami: `thera-rest-service` korzysta z Javy 25,
a `thera-payment-service` z Javy 21 (omówiono w rozdziale 3); różnica ta jest zachowana
w obu środowiskach uruchomieniowych.
...
|---|---|---|
| Język i środowisko uruchomieniowe | Java 25/21, Node 22 | brak |
...
MongoDB 4.2 (replikacja, wymagany TLS, przepustowość wyrażona w jednostkach RU/s,
ang. *Request Units per second*).
...
`start-dev` [X] z bazą PostgreSQL w kontenerze i importem realm z pliku, a konto administratora
...
|---|---|---|
| Przechowywanie | Plik tekstowy (wykluczony z systemu kontroli wersji) | Key Vault, szyfrowanie danych w spoczynku (ang. *at-rest*) |
    ...
    oauth2:
      resourceserver:
        jwt:
          ...
          # i weryfikuje podpis każdego tokenu Bearer — bez ręcznego dekodowania tokenu.
...
bez `fileReplacements` kompilacja produkcyjna użyłaby adresów deweloperskich.
...
W docelowym wdrożeniu komercyjnym klucz zostałby zastąpiony kluczem `pk_live_…`, a klucz tajny
(ang. *secret key*) po stronie `thera-payment-service` — odpowiednio `sk_live_…` — i przechowywany
wyłącznie w Azure Key Vault (rozdział 11).
...
> **Sugerowany podpis:** Rys. 10.5. Konfiguracja Spring w pliku `application.yml` z wartościami domyślnymi dla środowiska deweloperskiego
> **źródło:** opracowanie własne
...
Lokalna pętla informacji zwrotnej zamyka się w sekundach, a pełne wdrożenie do produkcji (budowa
obrazu, wypchnięcie do ACR, aktualizacja krocząca) zajmuje około 3–4 minut. Rozdzielenie
środowisk pozwala łączyć szybkość iteracji deweloperskiej z wiernością konfiguracji produkcyjnej.
...
| Zmiana skali | Nieuzasadniona (pojedyncza maszyna) | `--set <serwis>.replicas=N` (deklaratywnie) |
...
aplikacja działa na spójnym stosie technologicznym, a wszystkie różnice zamknięte są
